...
SpeakerProtectionTfa98xx::SpeakerProtectionTfa98xx()
    : isInitialized(false), speakerCount(0), powerAmpCount(0) {
  int status = 0;
...
void SpeakerProtectionTfa98xx::calibrationInfoInit() {
  ...
  powerAmpCount =
      std::min((speakerCount + 1) >> 1, static_cast<int>(MAX_PA_COUNT));

  PAL_INFO(LOG_TAG, "speakerCount:%d, powerAmpCount:%d", speakerCount,
           powerAmpCount);
  ...
  if (!hasCalibrationNodes) {
    PAL_INFO(LOG_TAG,
             "No calibration nodes found, using hardcoded impedance values");
    for (int i = 0; i < speakerCount; i++) {
      CaliInfo info = {};
      info.dev_idx = i;
      info.i2c_addr = DEVICE_ADDRESSES[i];
  ...
  if (!caliInfo.empty()) {
    ...
    for (auto &info : caliInfo) {
      ...
      PAL_INFO(LOG_TAG,
               "Speaker %hhx: addr=0x%x, impedance:(min=%dmΩ, max=%dmΩ, "
               "default=%dmΩ)",
               info.dev_idx, info.i2c_addr, info.min_imp, info.max_imp,
               info.def_imp);
    }
  }
}
...
void SpeakerProtectionTfa98xx::updateCalibrationValue() {
  ...
  for (auto &info : caliInfo) {
    ...
    if (calValue > info.min_imp && calValue < info.max_imp) {
      ...
      PAL_INFO(
          LOG_TAG,
          "Using fallback impedance for i2c=0x%x, dev_idx=%hhx: cal_imp=%d",
          info.i2c_addr, info.dev_idx, info.cal_imp);
    }

    PAL_INFO(
        LOG_TAG,
        "Speaker i2c=0x%x dev_idx=%i: impedance values: cal=%dmΩ, dsp=0x%x",
        info.i2c_addr, info.dev_idx, info.cal_imp, info.dsp_imp);
  }
}

// References: OplusSpeakerTfa98xx::payloadSPConfig and
// tfa98xx_adsp_send_calib_values() from tfa98xx_v6.c
void SpeakerProtectionTfa98xx::payloadSPConfig(uint8_t **payload, size_t *size,
                                               uint32_t miid) {
  ...
  *size = 0;
  ...
  if (caliInfo.empty() || caliInfo.size() != speakerCount) {
    PAL_ERR(LOG_TAG, "Invalid calibration info size: %zu, expected: %d",
            caliInfo.size(), speakerCount);
    return;
  }
...
FILE *SpeakerProtectionTfa98xx::openDeviceFile(uint8_t i2c_addr,
...
long SpeakerProtectionTfa98xx::readDeviceFile(char *buffer, size_t size,
                                              FILE *fp) {
  if (!buffer || !fp || size == 0)
    return 0;
...
int32_t SpeakerProtectionTfa98xx::sendPcmIdAndMiidToDriver(uint32_t miid,
                                                           int pcmId) {
